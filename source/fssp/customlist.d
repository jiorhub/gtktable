...
class CustomList : TreeModel
{
    ...
    void initColumns(Json meta)
    {
        columnTypes[0] = GType.POINTER;

        uint i;
        foreach (Json col; meta["columns"])
        {
            ++i;
            string type = col["type"].get!string;
            switch (type)
            {
                case "String":
                case "Date":
                    columnTypes[i] = GType.STRING;
                    break;
                case "Money":
                    columnTypes[i] = GType.DOUBLE;
                    break;
                default:
                    break;
            }
            names[i] = col["name"].get!string;
        }

        nColumns = cast(int)columnTypes.length;
    }
    ...
    override Value getValue(TreeIter iter, int column, Value value = null)
    {
        ...
        else
        {
            ...
            if (columnTypes[column] == GType.STRING)
                value.setString(record.data[name].get!string);
            else if (columnTypes[column] == GType.DOUBLE)
            {
                Json r = record.data[name];
                if (Json.Type.int_ == r.type)
                    value.setDouble(r.get!long);
                else if (Json.Type.float_ == r.type)
                    value.setDouble(r.get!float);
                else if (Json.Type.bigInt == r.type)
                    value.setDouble(r.get!long);
            }    
        }
